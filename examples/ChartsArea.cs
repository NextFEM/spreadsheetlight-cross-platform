...
namespace ConsoleApp
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            chart.SetChartType(SLAreaChartType.Area);
            chart.SetChartPosition(1, 9, 1 + fChartHeight, 9 + fChartWidth);
            sl.InsertChart(chart);

            chart = sl.CreateChart("B2", "G6");
            chart.SetChartType(SLAreaChartType.StackedAreaMax);
            chart.SetChartStyle(SLChartStyle.Style42);
            chart.SetChartPosition(7, 1, 7 + fChartHeight, 1 + fChartWidth);
            sl.InsertChart(chart);

            chart = sl.CreateChart("B2", "G6");
            chart.SetChartType(SLAreaChartType.Area3D);
            chart.SetChartStyle(SLChartStyle.Style35);
            chart.SetChartPosition(16, 9, 16 + fChartHeight, 9 + fChartWidth);
            sl.InsertChart(chart);

            sl.SaveAs("ChartsArea.xlsx");
