...
namespace ConsoleApp
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            chart.SetChartType(SLDoughnutChartType.Doughnut);
            chart.SetChartPosition(1, 9, 1 + fChartHeight, 9 + fChartWidth);
            sl.InsertChart(chart);

            chart = sl.CreateChart("B2", "G6");
            chart.SetChartType(SLDoughnutChartType.ExplodedDoughnut);
            chart.SetChartStyle(SLChartStyle.Style16);
            chart.SetChartPosition(7, 1, 7 + fChartHeight, 1 + fChartWidth);
            sl.InsertChart(chart);

            sl.SaveAs("ChartsDoughnut.xlsx");
