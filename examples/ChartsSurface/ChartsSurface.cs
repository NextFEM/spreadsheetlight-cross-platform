...
namespace ConsoleApp
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            chart.SetChartType(SLSurfaceChartType.Surface3D);
            chart.SetChartPosition(1, 9, 1 + fChartHeight, 9 + fChartWidth);
            sl.InsertChart(chart);

            chart = sl.CreateChart("B2", "G6");
            chart.SetChartType(SLSurfaceChartType.WireframeSurface3D);
            chart.SetChartStyle(SLChartStyle.Style36);
            chart.SetChartPosition(7, 1, 7 + fChartHeight, 1 + fChartWidth);
            sl.InsertChart(chart);

            chart = sl.CreateChart("B2", "G6");
            chart.SetChartType(SLSurfaceChartType.Contour);
            chart.SetChartStyle(SLChartStyle.Style37);
            chart.SetChartPosition(16, 9, 16 + fChartHeight, 9 + fChartWidth);
            sl.InsertChart(chart);

            sl.SaveAs("ChartsSurface.xlsx");
